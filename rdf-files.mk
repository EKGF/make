...
USE_CASE_FILES := $(shell find -L $(GIT_ROOT)/use-case -mindepth 1 -a -name '*.ttl' -a \! -path "./.git*" -print 2>/dev/null)
DATASET_NT_FILES := $(shell find -L $(GIT_ROOT)/static-dataset -mindepth 1 -a -name '*.nt' -a \! -path "./.git*" -print 2>/dev/null)
DATASET_TTL_FILES := $(shell find -L $(GIT_ROOT)/static-dataset -mindepth 1 -a -name '*.ttl' -a \! -path "./.git*" -print 2>/dev/null)
